...
// Store
import { changeRegisterServices } from '../../Store/actions/actionRegister'
...
/**
 * @deprecated
 * @description No momento foi decidio nao definir servico, unico é carro.
 */
class RegisterServicesStepScreen extends Component {
	constructor(props) {
		super(props)
	...
	componentDidMount() {
		BackHandler.addEventListener("backPress", () => {
			this.navigateToScreen("RegisterAddressStepScreen")
			return true
		})
	...
	getServicesInformation() {
		...
		if (this.props.services.length > 0) {
			...
			if (this.props.arrayRegisterServices.length > 0) {
				for (let i = 0; i < this.props.arrayRegisterServices.length; i++) {
					datasource.map(el => {
						if (el.id == this.props.arrayRegisterServices[i].id) {
							el.selected = true
							if (el.categories.length > 0) {
								for (let j = 0; j < this.props.arrayRegisterServices[i].categories.length; j++) {
									el.categories.map(cat => {
										if (cat.id == this.props.arrayRegisterServices[i].categories[j].id) {
											cat.selected = true
										}
									})
								}
	...
	/**
	 * Function to create the provider's model and navigate to next step
	 * @param {json} json (JSON with Provider Information from POST response)
	 */
	CreateLocalProviderAndNavigate(json, provider_type) {
		const { navigate } = this.props.navigation;
		this.setState({ isLoading: false });

		/**
		 * Update the Provider register step and
		 * Save model information into local Storage
		 */
		if (json) {
			this.provider.setRegister_step(json.register_step);
		} else {
			this.provider.setRegister_step("services");
		}

		this.provider.setProvider_type(provider_type);
		this.provider.store(constants.PROVIDER_STORAGE);

		navigate("RegisterVehicleStepScreen", { provider: this.provider });
	}
	...
	async RegisterService() {
		...
		if (resultArray.length > 0) {
			...
			try {
				const response = await this.apiProvider.RegisterServices(this.props.addProviderId, origin, resultArray,
					this.props.latitude,
				...
				if (parse.isSuccess(responseJson, this.returnConstNavigate()) == true) {
					...
					this.props.changeRegisterServices(resultArray)
		...
		return (
			<View>
				<CheckBox
					...
					containerStyle={[styles.contCheck, { marginTop: 10 }]}
				...
				<FlatList
					...
					renderItem={({ item, index }) => (
						<View style={[styles.areaRadio, { flexDirection: "row", alignItems: 'center' }]}>
							<Image
							...
							<CheckBox
								...
								containerStyle={[styles.contCheck, { paddingHorizontal: 0 }]}
								checkedColor={projectColors.green}
	...
	render() {
		return (
          ...
          <DefaultHeader
            ...
            btnBack={true}
            btnBackListener={() => this.props.navigation.navigate('RegisterAddressStepScreen')}
            ...
            subtitle={strings("register_steps.select_services")}
          />
...
const mapStateToProps = state => (
	{
		...
		arrayRegisterServices: state.registerReducer.arrayRegisterServices,
		servicesFilled: state.registerReducer.servicesFilled,
